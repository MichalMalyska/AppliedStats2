...
stan_df <- tibble(oi = observe.i, ei = log(expect.i), mi = aff.i) %>%
  mutate(mi = scale2(mi))

bigN <- nrow(stan_df)

stan_df$region <- 1:bigN

stan_data <- list(N = bigN,
                  x = stan_df$mi,
                  offset = stan_df$ei,
                  deaths = stan_df$oi,
                  region = stan_df$region)
```
...
fit1 <- stan(data = stan_data,
             file = here("code/models/theta_same.stan"),
             iter = 1000,
             seed = 2718)

fit2 <- stan(data = stan_data,
             file = here("code/models/theta_diff_region.stan"),
             iter = 1000,
             seed = 2718)

fit3 <- stan(data = stan_data,
             file = here("code/models/theta_diff_hierarch.stan"),
             iter = 1000,
             seed = 2718)
...
print(names(rstan::extract(fit1)))

fit1_params <- as_tibble(as.data.frame(fit1))

print(names(rstan::extract(fit2)))

fit2_params <- as_tibble(as.data.frame(fit2))

print(names(rstan::extract(fit3)))

fit3_params <- as_tibble(as.data.frame(fit3))
...
theta_mod1 <- mean(fit1_params$alpha)

thetas_mod2 <- rep(NA, bigN)

thetas_mod3 <- rep(NA, bigN)

for (i in 1:bigN) {
  alphas <- fit2_params[paste0("alpha[", i, "]")]
  betas <- fit2_params["beta"]
  xi <- stan_df %>% filter(region == i) %>% pull(mi)
  thetas <- alphas + betas * xi
  thetas_mod2[i] <- mean(as_vector(thetas))
}

for (i in 1:bigN) {
  alphas <- fit3_params[paste0("alpha[", i, "]")]
  betas <- fit3_params["beta"]
  xi <- stan_df %>% filter(region == i) %>% pull(mi)
  thetas <- alphas + betas * xi
  thetas_mod3[i] <- mean(as_vector(thetas))
}

plot_data <- tibble(mod1_theta = exp(theta_mod1), mod2_theta = exp(thetas_mod2), mod3_theta = exp(thetas_mod3), region = 1:bigN, smr = observe.i / expect.i)
```
...
plot_data %>%
  pivot_longer(cols = starts_with("mod"),
               names_to = "model",
               values_to = "theta") %>%
  mutate(model = str_sub(model, end = 4)) %>%
  ggplot(aes(x = smr, y = theta, color = model)) +
  geom_point(alpha = 0.6) +
  geom_abline(slope = 1, intercept = 0) +
  theme_bw() +
  labs(title = "Predicted Theta vs observed SMR for the three models")

```

```{r plots2 ,fig.height=8}
thetas_1 <- spread_draws(fit1, alpha) %>% mutate(theta = exp(alpha)) %>% select(-alpha)
thetas_2 <- spread_draws(fit2, log_theta[i]) %>% mutate(theta = exp(log_theta)) %>% select(-log_theta)
thetas_3 <- spread_draws(fit3, log_theta[i]) %>% mutate(theta = exp(log_theta)) %>% select(-log_theta)

thetas_2 %>%
  median_qi(.width = 0.95) %>%
  ggplot(aes(x = i, y = theta)) +
  geom_pointinterval() +
  theme_bw() +
  labs(title = "Estimated Thetas by region from model 2")
...
thetas_3 %>%
  ...
  geom_pointinterval() +
  theme_bw() +
  labs(title = "Estimated Thetas by region from model 3")
...
thetas_1 %>%
  ...
  geom_pointinterval() +
  theme_bw() +
  labs(title = "Estimated Theta by region from model 1")
```
