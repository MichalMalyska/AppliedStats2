...
```{r plots9}
cor <- df %>% select_if(is.numeric) %>% cor()
...
```{r visualizing poiss reg}

GGally::ggcoef(poiss_mod, exponentiate = TRUE,exclude_intercept = TRUE, vline_intercept = )
...
a lot more than the youth in Washington. 

## d) Poisson Regression - expected_deaths

```{r poiss reg expected deaths offset}
...
(Not 100% since I included the expected deaths variable)
...
res <- df %>%
	mutate(resids = resid(poiss_mod, type = "pear"))
...
```{r overdisp factor}
# Doesn't work for some ungodly reason
# n <- length(df$year)
# k <- length(coef(poiss_mod2))
# 
# overdisp_factor1 <- sum(res$resids^2) / (n - k)
...
P-value is pretty much zero so NB is much better. This is also very clear from
